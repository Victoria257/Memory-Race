  ...
  return (
    <div id="selection-panel" className="flex-grow h-full min-h-screen tablet:min-h-0 bg-[#F1F8E9] rounded-none tablet:rounded-3xl shadow-xl p-6 border-0 tablet:border-4 border-[#7DA33C]/20 transition-all duration-300 flex flex-col justify-center">
        ...
        <div className="grid grid-cols-3 tablet:grid-cols-4 gap-3">
          {categories.map(c => (
            <button
              ...
              className={`p-2 tablet:p-3 rounded-2xl border-4 flex flex-col items-center justify-center transition-all transform
            ...
            >
              <span className="text-2xl tablet:text-3xl mb-2">{c.icon}</span>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-4 tablet:flex tablet:flex-row tablet:flex-nowrap gap-3 tablet:gap-3 justify-items-center tablet:justify-between items-center py-6 px-4 bg-white/30 rounded-2xl overflow-x-auto scrollbar-hide">
          {colors.map(c => (
            <button
              key={c.id}
              disabled={!canSelect}
              onClick={() => setSelectedColor(c.id)}
              className={`w-10 h-10 tablet:w-14 tablet:h-14 rounded-full shadow-md transition-all flex items-center justify-center transform flex-shrink-0
                ${c.bg} 
      ...
      <button
        ...
        className={`w-full py-4 tablet:py-5 rounded-2xl font-black text-xl transition-all flex items-center justify-center gap-3 transform
